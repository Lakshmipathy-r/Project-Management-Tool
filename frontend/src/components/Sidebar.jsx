...
const Sidebar = ({ user, tasks }) => {
  ...
  return (
    <>
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-sm text-purple-500 font-medium flex items-center gap-1'>
                <Sparkles className='w-3 h-3' /> Task mode: ON 🔥
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Mobile Drawer */}
      {mobileOpen && (
        <div className='fixed inset-0 z-40 bg-black bg-opacity-30' onClick={() => setMobileOpen(false)}>
          <div className={SIDEBAR_CLASSES.mobileDrawer} onClick={e => e.stopPropagation()}>
            ...
            <div className='flex items-center gap-3 mb-6'>
              ...
              <div>
                <h2 className='text-lg font-bold text-gray-800 mt-16'>Hey, {username}</h2>
                <p className='text-sm text-purple-500 font-medium flex items-center gap-1'>
                  <Sparkles className='w-3 h-3' /> Lets power up some tasks !!
                </p>
              </div>
            </div>
