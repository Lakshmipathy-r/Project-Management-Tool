...
const handleDelete = async () => {
  ...
  try {
    ...
    onRefresh?.(); // This should reload the tasks from backend
  } catch (error) {
    if (error.response && error.response.status === 401) onLogout?.();
...
const handleSave = async (updatedTask) => {
  try {
    const completed =
      updatedTask.completed === 'Yes' || updatedTask.completed === true ? true : false
    ...
    await axios.put(`${API_BASE}/${task._id || task.id}/gp`, payload, { headers: getAuthHeaders() })
  ...
  : '-'}
            ...
            <div className={TI_CLASSES.createdRow}>
              <Clock className='w-3 h-3 sm:w-2.5 sm:h-3.5'/>
